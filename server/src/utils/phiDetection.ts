...
// (request notes, delay notes). This is a safety net, not a guarantee: it errs
// toward catching the obvious identifiers (MRNs, SSNs, dates of birth) while
// tolerating ordinary transport instructions. Names are NOT reliably
// detectable, so the UI also warns the user not to enter them.
...
// Room numbers are 3-4 digits and floor codes are short, so legitimate
// transport data does not trip this.
